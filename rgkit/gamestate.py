...
class GameState(object):
    ...
    def add_robot(self, loc, player_id, hp=None, robot_id=None, **kwargs):
            ...
            if not is_guard:
                ...
                for other_loc in collisions[loc]:
                    if robot.player_id != self.robots[other_loc].player_id:
                        # RMP count kills
                        if 0 < robot_delta.hp_end <= damage:
                            kills[other_loc] += 1

                        damage_caused[other_loc] += min(damage, robot_delta.hp_end)
            ...
            for player_id, player_damage_map in enumerate(
                ...
                if player_id != robot.player_id:
                    for actor_loc, damage in player_damage_map.items():
                        ...
                        damage_caused[actor_loc] += min(damage, robot_delta.hp_end)
                        robot_delta.hp_end -= damage

            # account for suicides and deserters
            if actions[loc][0] in ('suicide', 'desert'):
                robot_delta.hp_end = 0
                # RMP killing yourself doesn't count as a kill
    ...
    def apply_delta(self, delta):
        ...
        team_deaths = [sum(di.hp_end <= 0 for di in delta if di.player_id == i) for i in range(2)]
        previous_teammates = [sum(di.hp > 0 for di in delta if di.player_id == i) for i in range(2)]
        current_teammates = [sum(di.hp_end > 0 for di in delta if di.player_id == i) for i in range(2)]
        locs = set()
        for delta_info in delta:
            ...
            locs.add(delta_info.loc_end)
            new_state.add_robot(
                loc=delta_info.loc_end,
                ...
                kills=delta_info.kills,
                team_deaths=team_deaths[delta_info.player_id],
                opponent_deaths=team_deaths[1 - delta_info.player_id],
                birthturn=robot_id is None,
                previous_teammates=previous_teammates[delta_info.player_id],
                previous_opponents=previous_teammates[1 - delta_info.player_id],
                teammates=current_teammates[delta_info.player_id],
                opponents=current_teammates[1 - delta_info.player_id],
            )
    ...
    def get_game_info(self, player_id, new_turn=False, new_game=False, game_over=False):
        game_info = AttrDict()
        ...
        game_info.new_turn = new_turn
        game_info.new_game = new_game
        game_info.game_over = game_over
