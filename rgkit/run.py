...
class Options(object):
    def __init__(self, map_filepath=None, headless=False, print_info=False,
                 ...
                 n_of_games=1, start=0, autorun=False):
...
class Runner(object):
    def __init__(self, players=None, player_files=None, settings=None,
        ...
        # don't use symmetric spawn if the map isn't symmetric
        for loc in settings.spawn_coordinates:
            if (settings.board_size - 1 - loc[0], settings.board_size - 1 - loc[1]) not in settings.spawn_coordinates:
                if options.symmetric:
                    print('Warning: chose symmetric spawn for asymmetric map. Setting symmetric=False.')
    ...
    def play(self, match_seed):
        ...
        if not self.options.headless and not self.options.curses:
            ...
            #                                    if animate_render
            #                                    else "without")
            render.Render(g, self.options.animate_render, autorun=self.options.autorun, names=self._names)
        else:
            g.run_all_turns()
...
def get_arg_parser():
    ...
    default_map = str(importlib.resources.files('rgkit') / 'maps/default.py')
...
-qqqq: final summary only""")
    ...
    parser.add_argument("--game-version", default=None,
                        help="Version of game F=only moves, G=moves+guard, H=moves+suicide, I=moves+guard+suicide, "
                             "J=moves+attacks.")
