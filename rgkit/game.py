...
class Player(object):
    ...
    def _get_response(self, game_info, robot, seed, game_version):
        ...
        try:
            ...
            print(f'player {self._player_id+1}: {e}', file=sys.stderr)
            action = ['desert']
    ...
    def get_responses(self, game_state, seed, game_version):
        """
        ...
        fake_robot = dict(location=(10, 10), player_id=self._player_id, robot_id=0, hp=50)
        if game_info.turn == 0:
            # call act once for new game
            game_info.new_game = True
            for k, v in fake_robot.items():
                setattr(self._robot, k, v)
            self._robot.act(game_info)
        elif 0 < game_info.turn <= settings.max_turns:
            # call act once for new turn
            game_info.new_turn = True
            for k, v in fake_robot.items():
                setattr(self._robot, k, v)
            self._robot.act(game_info)

            game_info.new_turn = False
            ...
            assert game_info.turn == settings.max_turns+1
            # call act once more for this player to do bookkeeping
...
class Game(object):
    def __init__(self, players, record_actions=False, record_history=False,
                 ...
                 symmetric=True, game_version=None):
    ...
    def _get_robots_responses(self):
        ...
        for player in self._players:
            ...
            responses = player.get_responses(self._state, seed, game_version=self.game_version)
    ...
    def run_all_turns(self):
        ...
        # RMP call robot act one more time for each player to get results of last move
        self._get_robots_responses()
        # RMP call again to do end of game bookkeeping
        self._state.turn += 1
        self._get_robots_responses()
